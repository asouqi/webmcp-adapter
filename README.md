...
A lightweight utility designed to register, validate, and manage tool lifecycles within the `navigator.modelContext` environment. By leveraging the official [@mcp-b/webmcp-types](https://github.com/WebMCP-org/npm-packages) for its type system, the library automatically infers TypeScript argument types directly from your JSON schemas. This ensures your tools remain strictly compliant with the Model Context Protocol while providing a seamless, "zero-config" developer experience for modern web AI applications.
